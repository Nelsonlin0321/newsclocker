...
const getInstruction = ({ userPrompt }: { userPrompt: string }) => {
  const template = `
...
Redirect return optimized prompt only without any introducing.
...
export const optimizePrompt = async ({
  ...
  const userContent = getInstruction({ userPrompt: description });
